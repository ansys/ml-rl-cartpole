...
class CartPoleMapdlSimple:
    ...
    def __init__(self, **kwargs):
        ...
        self._cart_velocity_hint = self._preferences.get(
            'cart_velocity_hint', .05)
        self._theta_deg_hint = self._preferences.get(
            '_theta_deg_hint', np.degrees(.05))
        ...
        self._pole = Pole(self._preferences.get(
            'pole_radius', .01), self._preferences.get('pole_length', 1.))
        ...
        self._nodes['beam_end'] = self._nodes['beam_begin'] + \
            self._n_beam_elements
    ...
    def _reset_state(self):
        ...
        self._cart_velocity = self._cart_velocity_0 = \
            np.random.uniform(-self._cart_velocity_hint,
                              self._cart_velocity_hint)
    ...
    def reset(self):
        ...
        with mapdl.non_interactive:
            ...
            mapdl.trnopt('FULL')
            mapdl.ic(self._nodes['beam_begin'], "ROTY", #'vx', # VX cannot be in MPC184??
                    self._cart_velocity_0)    # initial conditions

    def _build_pole(self):
        # ## pole
        # Young's modulus in Pa
        mapdl.mp('EX', self._ids['pole'], self._youngs)
        # Poisson's ratio
        mapdl.mp('NUXY', self._ids['pole'], self._poisson)
        ...
        # beam elements
        mapdl.et(self._ids['pole'], 188)
        # create a beam element
        mapdl.e(self._nodes['beam_begin'], self._nodes['beam_begin'] + 1)
        # reproduce the pattern of the last element
        mapdl.egen(self._n_beam_elements, 1, -1)
    ...
    def _build_cart_mass_point(self):
        ...
        # rotary intertia options: 3-D mass without rotary inertia
        mapdl.keyopt(self._ids['cart'], 3, 2)
        # total mass (when keyopt(3) = 2, as is the case)
        mapdl.r(self._ids['cart'], self._cart_mass)
        mapdl.type(self._ids['cart'])
        mapdl.real(self._ids['cart'])
        mapdl.mat(self._ids['cart'])
        mapdl.secnum(self._ids['cart'])
        mapdl.n(node=self._mass['nodes'])
        mapdl.e(self._mass['nodes'])
    ...
    def _build_pole_joint_to_ground(self):
        ...
        mapdl.e(self._mass['nodes'], self._nodes['beam_begin'])
    ...
    def step(self, force_dir):
        ...
        with mapdl.non_interactive:
            ...
            mapdl.com(f"Time step: {self._cur_time_point}")

            # the global cs is accelerated upwards (so a gravity is felt downwards)
            mapdl.acel(0., 9.80665, 0.)
            mapdl.f(self._nodes['beam_begin'], 'fx',
                    self._force_dir * self._force_mag)
    ...
    def _query_state(self):
        ...
        node_to_evaluate = self._mass['nodes']

        self._cart_pos = self._cart_pos_0 + \
            mapdl.get_value('node', node_mass, 'u', 'x')

        self._cart_velocity = mapdl.get_value('node', node_mass, 'v', 'x')

        self._theta_deg = self._theta_deg_0 - \
            np.degrees(mapdl.get_value('elem', elem_top, 'SMISC', 66))

        self._pole_velocity = mapdl.get_value('node', node_top, 'v', 'sum')

        # self._reported_time_point = int(
        #     np.round(mapdl.get_value('active', 0, 'SET', 'LSTP')))  # Ger: maybe we should retrieve time

        self._reported_time_point = int(
            mapdl.get_value('active', 0, 'SET', 'TIME')/self._D_time)

        if self._cur_time_point != self._reported_time_point or \
                self._cur_time_point > self._max_time_points:
            print(f"Finishing!\t{self._cur_time_point} !={self._reported_time_point}\t{self._cur_time_point} > {self._max_time_points}")
            self._finished = True

        if np.abs(self._theta_deg) > self._theta_deg_max:
            print("Theta is too big")
            self._finished = True

        # print(np.array([self._cart_pos, self._cart_velocity, self._theta_deg, self._pole_velocity, self._reported_time_point],
        #                dtype=np.float))
